...
#transforms data to one hot
def to_one_hot(data, top = None):
    max = top if top is not None else np.max(data)  
    tData = np.zeros((max, 0))

    for i in range(data.shape[1]):
        curr = np.array(one_hot(data[0, i], max))
        tData = np.concatenate((tData, curr), axis=1)
...
# tData = to_one_hot(data)

# for i in range(data.shape[0]):
#     curr = np.array(one_hot(data[i, 0] , 6))
#     tData = np.concatenate((tData, curr), axis=1)

# print(tData)
# print(labels)
# print(perceptron(tData, labels))
...
##########################################################################3
# test for 3
sets = [super_simple_separable_through_origin, super_simple_separable, xor, xor_more]
degrees = []
for set in sets:
    i = 1
    cont = True
    while cont:
        test_with_features(set, i, pause=False, draw=False)
        i += 1
