...
class ReLU(Module):  # Layer activation
    def __init__(self):
        self.m = lambda z : max(0, z)
        self.dm = lambda r : 0 if r <= 0 else 1

    def forward(self, Z):
        self.A = np.vectorize(self.m)(Z)  # Your code: (?, b)
        return self.A

    def backward(self, dLdA):  # uses stored self.A
        return dLdA * np.vectorize(self.dm)(self.A)  # Your code: return dLdZ (?, b)
...
def nn_tanh_test(iters=1):
    ...
    X, Y = super_simple_separable()
    print("helo")
    nn.mini_gd(X, Y, iters=iters, lrate=0.005)
    print("goodbye")
