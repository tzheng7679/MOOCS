...
def plot_separator(ax, th, th_0):
    ...
    if abs(th[1,0]) > eps:
        pts += [np.array([x, (-th_0 - x * th[0,0]) / th[1,0]], object) \
                                                        for x in (xmin, xmax)]
    if abs(th[0,0]) > 1.0e-6:
        pts += [np.array([(-th_0 - y * th[1,0]) / th[0,0], y], object) \
                                                         for y in (ymin, ymax)]
    in_pts = []
    for p in pts:
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    for t in range(T):
        for i in range(n):
            ...
            if hook is not None:
                nT = t * n + i + 1
                print((ths.T / tot, th0s / tot))
                hook((ths.T / tot, th0s / tot))
    return (ths.T / tot, th0s / tot)
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        ...
        totScore += eval_classifier(learner, data_train, labels_train, data_test, labels_test)
...
def xval_learning_alg(learner, data, labels, k):
    ...
    (split_data, split_labels) = (np.array_split(data,k,axis=1), np.array_split(labels,k,axis=1))
    sco = 0
    
    for exclude in range(k):
        testIndices = [i for i in range(exclude)] + [i for i in range(exclude + 1, k)]
        test_data = np.concatenate([split_data[i] for i in testIndices], axis = 1)
        test_labels = np.concatenate([split_labels[i] for i in testIndices], axis = 1)

        (train_data, 
         train_labels) = (split_data[exclude], split_labels[exclude])
        
        sco += eval_classifier(learner, train_data, train_labels, test_data, test_labels)
...
# print("Perceptron\n---------------")
# print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.5), 20, 20, 20))
# print(eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 20))
# print("--------------------------")
# print("Averaged Perceptron\n---------------")
# print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.5), 20, 20, 20))
# print(eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 20))

#hw3 additions
perceptron(data = np.array([[.2, .8, .2, .8],
             [0.2,  0.2,  0.8,  0.8]]), labels = np.array([[-1, -1, 1, 1]]))
