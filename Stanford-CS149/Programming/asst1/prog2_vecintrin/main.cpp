...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
    ...
    //
    __cs149_vec_float max = _cs149_vset_float(9.999999);
    __cs149_vec_int ones = _cs149_vset_int(1);
    __cs149_vec_int zeros = _cs149_vset_int(0);

    int N_registers = N / VECTOR_WIDTH + (N % VECTOR_WIDTH > 0);
    __cs149_vec_float output_registers[N_registers];
    __cs149_vec_float value_registers[N_registers];
    __cs149_vec_int exp_registers[N_registers];
    __cs149_mask masks[N_registers];

    Logger l;

    for(int i = 0; i < N_registers; i++) {
        /* initialize serial data in vector memory */
        // set registers size in case (VECTOR_WIDTH \ndiv N)
        int register_size = VECTOR_WIDTH;
        if(i == N_registers - 1) { // if last mask, match mask size with N % VECTOR_WIDTH
            register_size = N - (i) * VECTOR_WIDTH;
        }
        __cs149_mask load_mask = _cs149_init_ones(register_size); // create the mask for loading
        // initialize mask
        masks[i] = _cs149_init_ones(register_size);

        // load from values & exponents and reset mask
        _cs149_vload_float(value_registers[i], &(values[i*VECTOR_WIDTH]), load_mask);
        _cs149_vload_int(exp_registers[i], &(exponents[i*VECTOR_WIDTH]), load_mask);
        output_registers[i] = _cs149_vset_float(1.f);

        /* exponentiation */
        while(_cs149_cntbits(masks[i]) > 0) {
            // check for where exponent is greater than 0
            _cs149_vgt_int(masks[i], exp_registers[i], zeros, masks[i]);

            // multiply element-wise
            _cs149_vmult_float(output_registers[i], output_registers[i], value_registers[i], masks[i]);
            
            // update exponents
            _cs149_vsub_int(exp_registers[i], exp_registers[i], ones, masks[i]);
        }

        /* clamping */
        __cs149_mask clamp_mask;
        _cs149_vgt_float(clamp_mask, output_registers[i], max, load_mask);
        _cs149_vset_float(output_registers[i], 9.999999, clamp_mask);

        /* move from vectors back to serial */
        __cs149_mask store_mask = _cs149_init_ones(register_size);
        _cs149_vstore_float(&output[i*VECTOR_WIDTH], output_registers[i], store_mask);
    }
}
...
float arraySumVector(float* values, int N) {
  ...
  //
  __cs149_mask load_mask = _cs149_init_ones(N);

  float sum = 0;
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    __cs149_vec_float vec;
    _cs149_vload_float(vec, &(values[i]), load_mask);

    // do addition by adding and interleaving
    ...
    _cs149_vstore_float(results, vec, load_mask);
